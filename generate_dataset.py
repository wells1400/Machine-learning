...
def mk_data(N, noisy=False):
    ...
    X=np.random.rand(dim,N)*(rang[1]-rang[0])+rang[0] # dim * N 维矩阵，元素在rang范围

    while True:
        # (dim+1) * dim 维矩阵 首行为1，其余元素在rang范围
        Xsample = np.concatenate((np.ones((1,dim)), np.random.rand(dim,dim)*(rang[1]-rang[0])+rang[0]))
        k,w=null(Xsample.T) #  传入dim *(dim+1) ,首列为1
...
def generate_dataset(size, noisy = False, visualization = True):
    global sign
    sign = np.vectorize(sign)   #sign是一个全局向量
